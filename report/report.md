...
> (`scripts/`, results in `results/`). Nothing is hand-typed. Repo URL: [shiinayane/visual-media-gres](https://github.com/shiinayane/visual-media-gres).
...
1. **MSDeformAttn fallback.** The hand-written `MultiScaleDeformableAttention` CUDA op cannot be
   ...
   **close** (not bit-identical — `grid_sample` boundary handling and fp accumulation differ from
   the hand-written CUDA kernel); we attribute the ~1 pt baseline gap below partly to this.
...
We study two failure conditions on gRefCOCO's official splits, so every claim is a measured rate,
...
we therefore **hypothesise** that when even a few regions confidently "match" a salient distractor
that loosely fits the words, the mean is pulled below 0.5 and a mask is committed. (We did not dump
per-region NT logits, so this averaging-dilution account is *consistent with* the architecture but
not directly measured — see §8.) Figure `F1_halluc_area_hist.png` shows these hallucinations are
sizeable, not slivers. This is exactly the behaviour that is unsafe for a robot acting on language.
Qualitative cases (GT empty, prediction non-empty): `report/figures/qual_nt_hallucination_swin_tiny.png`.
...
**Setup.** We analyse **testA** here, because — unlike val, whose targeted expressions are *all*
multi-instance — testA contains a **single-instance baseline** (5 917 one-target, 5 940 two-target,
2 895 three-or-more expressions), so we can ask whether multi-target is genuinely harder than
single-target. For each GT instance we record coverage = |pred ∩ inst|/|inst|, call it a *hit* if
...
per-region minimap weights. A single mask comfortably represents one — or, with the soft region
weighting, two — salient regions, but cannot spread mass across many disjoint instances, which is
exactly where recall falls off a cliff (≥3) and why the weaker instance in a pair is the one
sacrificed. Qualitative cases (GT instances in green, prediction in red — the red mask frequently
covers only the more salient instance): `report/figures/qual_multi_target_swin_tiny.png`.
...
**Read the val row as a calibration curve, not a result.** τ is both *chosen* and *scored* on val,
so the val gains are **in-sample and optimistic** by construction (on val the safe point shows
gIoU +11.2 / cIoU +2.6 / N-acc +19, the gIoU-optimal point gIoU +18). We report them only to show
the shape of the trade-off; the honest evaluation is the **held-out testA**.

**On held-out testA the detector is a safety re-weighting, not a free overlap gain — stated
plainly.** Transferring τ unchanged: the **safe** τ=0.38 raises **N-acc by +12.7 points**
(50.6→63.3) at an **8.4-point T-acc cost** (99.0→90.6), while the overlap metrics are essentially
**flat to slightly negative** (gIoU +0.9 — within the ~1 pt reproduction noise — and cIoU −1.0).
In other words the detector does **not** improve segmentation quality on a target-heavy split; what
it buys is a tunable, calibrated **willingness to abstain** (catch more unsatisfiable instructions)
at a controlled, bounded cost in target retention — precisely the act-vs-ask trade a robot needs.
The aggressive gIoU-optimal τ=0.23 is **worse**: it was tuned to val's 63 %-empty prior and on the
23 %-empty testA it over-abstains and **drops gIoU by 11** (65.0→54.1). The clean lesson — itself a
useful safety finding — is that a single global abstention threshold is **sensitive to the
no-target base-rate** of the deployment distribution: only the T-acc-constrained operating point
transfers safely, and a deployable detector should calibrate τ to the expected base-rate (or be made
prior-independent). This sharpens, rather than weakens, the case for scoring and operating grounding
models *with* an explicit abstain option.
...
multi-component masks where "ask which one?" is a more appropriate robot behaviour than silently
committing to one blob — an illustrative remedy for F2 (we count the trigger but, lacking a
clarification label in gRefCOCO, do not claim a precision for it; see §8). Qualitative flips (a
confident wrong mask on an absent referent becomes a correct abstention):
...
- The **grid_sample fallback** for MSDeformAttn is the reference op (numerically *close*, not
  identical) but slower; we did not compile the custom CUDA op on Blackwell, so the ~1 pt baseline
  gap is plausibly-but-not-provably benign (no A/B against the compiled kernel).
- **F1 mechanism is hypothesised, not measured at the region level.** We record only the
  pooled `s_nt`; we did not dump per-region NT logits, so the "averaging-dilution by a few
  distractor regions" account is consistent with the architecture but not directly demonstrated.
- The abstain score `max(s_nt, 1−mask_conf)` is a simple, interpretable rule; the two signals are
  correlated, so it largely tracks `s_nt`. A learned calibrator over {s_nt, mask_conf, area,
  #components} would likely Pareto-dominate it but moves away from the "zero-training, transparent
  policy" goal.
- **The improvement is a safety re-weighting, not a segmentation-quality gain on held-out data**
  (testA gIoU ≈ flat, cIoU slightly down); its value is calibrated abstention, and a *single* global
  τ is sensitive to the no-target base-rate (the gIoU-optimal τ does not transfer across splits).
- The **CLARIFY** trigger is a geometric heuristic (#components, largest-fraction) and is reported
  only as a trigger **count** — we have no clarification ground truth in gRefCOCO, so its
  *precision* (genuine ambiguity vs a correct two-instance mask) is unmeasured. It also does not yet
  read the *expression* to distinguish "the two cups" (should segment both) from real ambiguity.
...
three actions a collaborator actually needs — act, abstain, ask. With no training it buys a
tunable, calibrated no-target recall (N-acc) at a bounded target-retention cost that **transfers to
a held-out split** (and a larger gIoU gain when the deployment distribution is no-target-heavy).
That supports a broader claim for VLA/HRI benchmarks: **grounding models should be scored, and
operated, with an explicit abstain/clarify option**, because a confident wrong mask is more
dangerous to a robot than an honest "I'm not sure — which one?". Natural next steps:
