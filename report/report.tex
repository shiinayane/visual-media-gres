...
% ---- single-column academic preamble; compile with LuaLaTeX ----
% Japanese is typeset by LuaTeX-ja (proper kinsoku line-breaking + Harano Aji
% Mincho, which ships with TeX Live, so it is reproducible without system fonts).
\usepackage[a4paper,top=0.9in,bottom=0.95in,left=1.15in,right=1.15in]{geometry}
...
% GitHub glyph loaded directly via fontspec (fontawesome5's macros clash with
% hyperref under luatexja); U+F09B is the GitHub mark in the FA5 Brands font.
\usepackage{fontspec}
\newfontface{\FABrands}{FontAwesome5Brands-Regular-400.otf}
\newcommand{\ghlogo}{{\FABrands\symbol{"F09B}}}
\usepackage{titlesec}
\usepackage{enumitem}
...
\ghlogo~\textbf{Code \& data:}~\href{https://github.com/shiinayane/visual-media-gres}%
{\texttt{github.com/shiinayane/visual-media-gres}}}}\\[0.3ex]
{\small Every number and figure regenerates from \code{scripts/} + \code{results/}, one inference
pass per split.}
\end{center}
...
\end{tabular}\\[2pt]
\textbf{Department:} 情報理工学系研究科 電子情報学専攻\\
\textbf{Laboratory:} 鈴村研究室 (Suzumura Lab)\\
...
Highlight}; network \textbf{ReLA}) generalizes classic Referring Expression Segmentation, which
assumes \emph{exactly one} target per expression. GRES additionally allows \emph{multi-target}
expressions (``the two bottles on the left'') and \emph{no-target} expressions, whose referent is
absent from the image. To support and measure this, the authors release \textbf{gRefCOCO} (COCO
train2014 images, RefCOCO-style expressions; train/val/testA/testB splits) together with an
evaluation protocol of five metrics:
\begin{itemize}
\item \textbf{gIoU} --- mean per-sample IoU (a correctly predicted no-target sample scores~1);
\item \textbf{cIoU} --- cumulative intersection over cumulative union;
\item \textbf{N-acc} --- no-target recall, $\mathrm{TP}/(\mathrm{TP}+\mathrm{FN})$;
\item \textbf{T-acc} --- target retention, $\mathrm{TN}/(\mathrm{TN}+\mathrm{FP})$;
\item \textbf{Pr@\{0.7,0.8,0.9\}} --- precision at IoU thresholds $0.7/0.8/0.9$.
\end{itemize}
...
I study two failure conditions on gRefCOCO's official splits.
F1 (no-target) is analysed on \textbf{val} (63\% of its samples are no-target---the
...
of val). A \emph{hallucination} is a no-target sample on which the model still commits to a mask---its
NT head predicts \emph{not} no-target ($s_\mathrm{nt}<0.5$); equivalently, the hallucination rate is
$1-$N-acc.
...
artefacts---median 7.2\% of the image (mean 9.1\%), at a mean foreground confidence of 0.68, well
above the $0.5$ commit threshold: these are not hesitant predictions.
...
acts on language, this high-confidence false positive is the most costly error; Fig.~\ref{fig:f1b}
shows representative cases.

\begin{figure}[t]
\centering
\includegraphics[width=0.58\linewidth]{F1_nt_score_hist.png}
\caption{No-target score $s_\mathrm{nt}$ for present vs absent referents. On absent referents the
score is bimodal; the large low mode collapses inside the present-target distribution, so 53\% fall
below $0.5$ and a mask is committed.}
\label{fig:f1a}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.5\linewidth]{qual_nt_hallucination_swin_tiny.png}
\caption{Qualitative no-target cases. Each row: one \emph{no-target} expression (left), the empty
ground truth (middle), and the model's confident hallucinated mask in red (right).}
\label{fig:f1b}
\end{figure}
...
multi-instance) it has a single-instance baseline: of its 19{,}200 expressions, 14{,}752 have a
target---5{,}917 one-target, 5{,}940 two-target, 2{,}895 three-or-more (the remaining 4{,}448 are
no-target). ReLA emits a \emph{single} union mask, so every GT instance of an expression is scored
against that one prediction: I record coverage $=|\mathrm{pred}\cap\mathrm{inst}|/|\mathrm{inst}|$,
call it a \emph{hit} if $\geq0.5$ (the conventional cutoff), and report per-instance recall.
...
\textbf{lopsided coverage} sacrificing the less salient instance (Fig.~\ref{fig:f2a}).
...
weighting is shared, but cannot spread mass across many disjoint instances---consistent with both the
sharp drop at $\geq$3 and the dropped weaker instance within a pair (an architectural account, not an
ablation); Fig.~\ref{fig:f2b} shows examples.

\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{F2_recall_by_count.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.52\linewidth]{qual_multi_target_swin_tiny.png}
...
Results on both splits are in Tables~\ref{tab:val}--\ref{tab:testa} (trade-off in
Fig.~\ref{fig:tradeoff}); $\tau$ is calibrated on \textbf{val} and transferred \textbf{unchanged} to
held-out \textbf{testA}.
...
calibrated willingness to abstain on unsatisfiable instructions at a capped loss in target retention.
The aggressive gIoU-optimal $\tau$ \emph{does not transfer}: tuned to val's 63\%-no-target prior, on
the 23\%-no-target testA it over-abstains and lowers gIoU by roughly 11 points. Hence only the
T-acc-constrained
...
alternative; a candidate remedy for F2, reported only as a trigger count since gRefCOCO has no
clarification label.

\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{abstain_tradeoff.png}
...
\item The detector is not compared against a naive abstention rule (e.g.\ abstaining by mask area
alone); it combines two model signals, and isolating each one's contribution is left to future work.
\end{itemize}
...
a referent exists, and how many. The detector adds no capacity; it reuses uncertainty the model
already encodes, turning one \code{argmax} into three actions (act, abstain, ask). The gain is modest
and prior-dependent, but it survives transfer to a held-out split. For a benchmark aimed at
human--robot use, this argues for an explicit abstain/clarify option in how grounding models are both
\emph{scored} and \emph{operated}. Natural next steps are a small learned calibrator over the four
signals, an expression-aware CLARIFY trigger, and an abstention-aware metric (e.g.\ risk--coverage)
reported alongside gIoU/cIoU.
