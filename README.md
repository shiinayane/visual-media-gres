...
> Report: `report/report.md` (export to PDF before submission). Repo URL: [shiinayane/visual-media-gres](https://github.com/shiinayane/visual-media-gres).
...
- **F2 (multi-target under-segmentation, testA):** per-instance recall is flat for 1–2 targets
  (0.87, 0.88) but **collapses to 0.59 at ≥3 targets**, and within a multi-target expression
  coverage is very uneven (best instance 0.92 vs worst 0.58 — an instance is dropped entirely 38 %
  of the time). One 2-channel mask saturates beyond ~2 regions.
- **Improvement:** a frozen-model abstain/clarify detector. On **held-out testA** the
  safety-constrained operating point (T-acc ≥ 0.95) is a **safety re-weighting**, not a free overlap
  gain: **N-acc +12.7** at an 8.4-pt T-acc cost, gIoU ≈ flat. The val numbers are in-sample
  (τ is calibrated there); the gIoU-optimal τ overfits val's no-target prior — all reported honestly
  in §7.
